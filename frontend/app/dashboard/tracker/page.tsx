  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[#1e2a4a]">Application Tracker</h1>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
          <div className="bg-white rounded-xl border border-[#e2e8f0] p-4">
            ...
            <p className="text-2xl font-bold text-[#1e2a4a] mt-1">{stats.total_applications}</p>
          </div>
          <div className="bg-white rounded-xl border border-[#e2e8f0] p-4">
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-[#e2e8f0] p-5 sm:p-6 mb-6">
          <h2 className="text-lg font-semibold text-[#1e2a4a] mb-4">Add New Application</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input type="text" placeholder="Job Title *" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full px-4 py-2.5 border border-[#e2e8f0] rounded-xl focus:ring-2 focus:ring-[#4f6ef7] text-sm bg-white text-[#1e2a4a] placeholder-[#9ca3af]" />
            <input type="text" placeholder="Company *" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="w-full px-4 py-2.5 border border-[#e2e8f0] rounded-xl focus:ring-2 focus:ring-[#4f6ef7] text-sm bg-white text-[#1e2a4a] placeholder-[#9ca3af]" />
            <input type="text" placeholder="Location" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="w-full px-4 py-2.5 border border-[#e2e8f0] rounded-xl focus:ring-2 focus:ring-[#4f6ef7] text-sm bg-white text-[#1e2a4a] placeholder-[#9ca3af]" />
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="w-full px-4 py-2.5 border border-[#e2e8f0] rounded-xl focus:ring-2 focus:ring-[#4f6ef7] text-sm bg-white text-[#1e2a4a]">
              {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
            </select>
            <textarea placeholder="Notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} className="sm:col-span-2 w-full px-4 py-2.5 border border-[#e2e8f0] rounded-xl focus:ring-2 focus:ring-[#4f6ef7] text-sm bg-white text-[#1e2a4a] placeholder-[#9ca3af]" />
          </div>
      ...
      {applications.length === 0 ? (
        ...
        <div className="space-y-3">
          {applications.map((app) => (
            <div key={app.id} className="bg-white rounded-xl border border-[#e2e8f0] p-4 sm:p-5">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-[#1e2a4a] text-sm sm:text-base">{app.job?.title || 'Unknown Role'}</h3>
