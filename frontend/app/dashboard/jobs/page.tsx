          ...
          {loading ? (
            ...
            jobs.map((job) => (
              ...
              >
                ...
                <p className="text-[#2d2d3f] text-xs font-medium mt-1">{job.company}</p>
          ...
          {selectedJob ? (
            <div className="bg-white rounded-2xl border border-[#eef0f5] shadow-sm p-5 sm:p-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {selectedJob.location && (
                      <span className="inline-flex items-center gap-1 text-sm text-[#6b7280] bg-[#f4f5f9] px-2.5 py-1 rounded-lg">
                    ...
                    {selectedJob.source && <span className="text-xs bg-[#eef1f8] text-[#2d2d3f] px-2.5 py-1 rounded-lg font-medium">{selectedJob.source}</span>}
                  </div>
                </div>
                <button
                  onClick={() => handleApply(selectedJob.id)}
                  disabled={applying}
                  className="bg-[#2d2d3f] hover:bg-[#1a1a2e] text-white font-semibold py-2.5 px-6 rounded-xl transition text-sm disabled:opacity-50 shadow-sm flex-shrink-0"
                >
              ...
              <div className="space-y-5">
                ...
                {selectedJob.url && (
                  <a href={selectedJob.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[#2d2d3f] hover:text-[#1a1a2e] text-sm font-semibold transition border-b border-[#2d2d3f]/30 hover:border-[#1a1a2e] pb-0.5">
